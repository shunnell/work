...
resource "aws_eip" "this" {
  count  = var.create_nat_gateway && var.eip_ids == null && length(var.public_subnets) > 0 ? var.nat_gateway_count : 0
  domain = "vpc"
...
resource "aws_nat_gateway" "this" {
  ...
  allocation_id = var.eip_ids != null ? var.eip_ids[count.index] : aws_eip.this[count.index].id
