...
inputs = {
  ...
  log_group_arns = [
    "arn:aws:logs:${include.account.locals.region}:${include.account.locals.account_id}:log-group:/aws/eks/dos-ca-opr-prod-cluster/cluster",
    "arn:aws:logs:${include.account.locals.region}:${include.account.locals.account_id}:log-group:/aws/containerinsights/dos-ca-opr-prod-cluster/dataplane"
  ]
}