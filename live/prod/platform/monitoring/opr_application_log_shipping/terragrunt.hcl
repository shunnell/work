...
dependency "cloudwatch_sharing_target" {
  config_path = "${get_path_to_repo_root()}/logs/platform/monitoring/cloudwatch_to_splunk_shipment_destinations/application"
  mock_outputs = {
...
inputs = {
  ...
  log_group_arns = [
    "arn:aws:logs:${include.account.locals.region}:${include.account.locals.account_id}:log-group:/aws/containerinsights/dos-ca-opr-prod-cluster/application",
    "arn:aws:logs:${include.account.locals.region}:${include.account.locals.account_id}:log-group:/aws/rds/cluster/dos-ca-prod-opr-v3/postgresql",
  ]
}
