...
locals {
  ...
  admin_vpc_vars = read_terragrunt_config(find_in_parent_folders("vpc.hcl")).locals
}

inputs = {
  vpc_name = "${local.admin_vpc_vars.common_identifier}-vpc"
  vpc_cidr = local.admin_vpc_vars.vpc_cidr_block
  availability_zones = [
    "us-east-1a",
    "us-east-1b",
    "us-east-1c",
