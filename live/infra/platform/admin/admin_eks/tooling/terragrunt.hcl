...
locals {
  network_locals = read_terragrunt_config("${get_path_to_repo_root()}/network/account.hcl").locals
  vpn_vars       = read_terragrunt_config("${get_path_to_repo_root()}/infra/platform/vpn/vpn_vars.hcl").locals
  vpc_vars       = read_terragrunt_config("../../admin_vpc/admin_vpc.hcl").locals
}

inputs = {
  allowed_cidr_blocks = [
    local.network_locals.bespin_cidr_block,
