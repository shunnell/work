...
dependency "dev_infra" {
  config_path = "../../permission_sets/Dev_Infra"
  mock_outputs = {
    permission_set_arn = "arn:aws:sso:::permissionSet/mock/Dev_Infra"
  }
}
locals {
  tenant_account_id = read_terragrunt_config("${get_repo_root()}/pqs/account.hcl").locals.account_id
...
inputs = {
  group_display_name = "Cloud City PQS Developer Enterprise Users"
  account_to_permission_set_map = tomap({
    (local.tenant_account_id) = dependency.sandbox_dev.outputs.permission_set_arn
    (local.infra_account_id)  = dependency.dev_infra.outputs.permission_set_arn
