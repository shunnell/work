...
locals {
  tenant_account_id = read_terragrunt_config("${get_repo_root()}/camp/account.hcl").locals.account_id
...
inputs = {
  group_display_name = "Cloud City CAMP Developer Enterprise Users"
  account_to_permission_set_map = tomap({
    (local.tenant_account_id) = dependency.sandbox_dev.outputs.permission_set_arn
    (local.infra_account_id)  = dependency.infra.outputs.permission_set_arn
