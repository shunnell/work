...
dependency "pass_observer" {
  config_path = "../../permission_sets/PASS_Observer"
  mock_outputs = {
    permission_set_arn = "arn:aws:sso:::permissionSet/mock-12345"
  }
}

dependency "pass_dev_infra" {
  config_path = "../../permission_sets/PASS_Dev_Infra"
  mock_outputs = {
    permission_set_arn = "arn:aws:sso:::permissionSet/mock-12345"
  }
}
...
inputs = {
  group_display_name = "Cloud City PASS Developer Enterprise Users"
  account_to_permission_set_map = tomap({
    (local.tenant_account_id) = dependency.sandbox_dev.outputs.permission_set_arn
    (local.infra_account_id)  = dependency.pass_dev_infra.outputs.permission_set_arn
    (local.prod_account_id)   = dependency.pass_observer.outputs.permission_set_arn
  })
}