...
dependency "visas_observer" {
  config_path = "../../permission_sets/Visas_Observer"
  mock_outputs = {
    permission_set_arn = "arn:aws:sso:::permissionSet/mock-12345"
  }
}

dependency "visas_dev_infra" {
  config_path = "../../permission_sets/Visas_Dev_Infra"
  mock_outputs = {
    permission_set_arn = "arn:aws:sso:::permissionSet/mock-12345"
  }
}
...
inputs = {
  ...
  account_to_permission_set_map = tomap({
    ...
    (local.infra_account_id)  = dependency.visas_dev_infra.outputs.permission_set_arn
    (local.prod_account_id)   = dependency.visas_observer.outputs.permission_set_arn
  })
}