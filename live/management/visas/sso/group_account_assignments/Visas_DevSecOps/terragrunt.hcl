...
dependency "visas_devsecops" {
  config_path = "../../permission_sets/Visas_DevSecOps"
  mock_outputs = {
    permission_set_arn = "arn:aws:sso:::permissionSet/mock-12345"
  }
}

dependency "visas_devsecops_infra" {
  config_path = "../../permission_sets/Visas_DevSecOps_Infra"
  mock_outputs = {
    permission_set_arn = "arn:aws:sso:::permissionSet/mock-12345"
  }
}
...
inputs = {
  group_display_name = "Cloud City Visas DevSecOps Enterprise Users"
  account_to_permission_set_map = tomap({
    (local.tenant_account_id) = dependency.sandbox_dev.outputs.permission_set_arn
    (local.infra_account_id)  = dependency.visas_devsecops_infra.outputs.permission_set_arn
    (local.prod_account_id)   = dependency.visas_devsecops.outputs.permission_set_arn
  })
}
