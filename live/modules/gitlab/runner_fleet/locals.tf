data "aws_secretsmanager_secret_version" "gitlab_secret" {
  ...
  lifecycle {
    postcondition {
      condition     = can(jsondecode(self.secret_string)[var.runner_fleet_name]["token"])
...
locals {
  ...
  # Note: cluster data variable is provided via terragrunt helm.tf
  join_token        = sensitive(jsondecode(data.aws_secretsmanager_secret_version.gitlab_secret.secret_string)[var.runner_fleet_name]["token"])
  runner_fleet_name = "gitlab-runners-${var.runner_fleet_name}"
  ...
  tags = merge(
    {
      ...
      "gitlab-runner-fleet" = var.runner_fleet_name
